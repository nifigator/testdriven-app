...
class TestUserService(BaseTestCase):
    ...
    def test_add_user_invalid_json_keys(self):
        """
        Ensure error is thrown if the JSON object does have a username key.
        """
        add_user('test', 'test@test.com', 'test')
        # update user
        user = User.query.filter_by(email='test@test.com').first()
        ...
        with self.client:
            ...
            response = self.client.post(
                '/users',
                data=json.dumps({'email': 'michael@mherma.org',
                                 'password': 'greaterthaneuight'}),
                content_type='application/json',
                headers={'Authorization': f'Bearer {token}'}
            )
            data = json.loads(response.data.decode())
    ...
    def test_single_user(self):
        ...
        user = add_user('michael', 'michael@mherman.org', 'greaterthaneuight')
        with self.client:
    ...
    def test_single_user_no_id(self):
        """Ensure error is return if an id is not provided."""
        with self.client:
    ...
    def test_all_users(self):
        ...
        add_user('fletcher', 'fletcher@notreal.com', 'graetthaneight')
        with self.client:
    ...
    def test_main_with_users(self):
        ...
        added to the database"""
        add_user('michael', 'michael@mherman.org', 'greaterthaneight')
        add_user('fletcher', 'fletcher@noreal.com', 'greaterthaneight')
        with self.client:
            response = self.client.get('/')
            self.assertEqual(response.status_code, 200)
            self.assertIn(b'All Users', response.data)
